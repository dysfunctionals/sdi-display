    ...
    def run(self):
        while True:
            ...
            if r.status_code == 200:
                remote_ships = r.json()
                for i in range(0, 4):
                    self.ships[i].power["engines"] = int(remote_ships[i]["engine"]["power"])
                    self.ships[i].power["shields"] = int(remote_ships[i]["shields"]["power"])
                    self.ships[i].power["weapons"] = int(remote_ships[i]["weapons"]["power"])
