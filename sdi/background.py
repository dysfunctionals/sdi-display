...
class Background:
    ...
    def __init__(self, screen):
        ...
        self.front_scroller = StarScroller(
            screen.get_width(), screen.get_height(), 2, 1, 5, 10
        )
        for x in range(self.front_scroller.sprite_amount):
            self.front_scroller.add_new(
                random.randint(0, screen.get_width()),
                random.randint(0, screen.get_height()),
            )

        self.middle_scroller = StarScroller(
            screen.get_width(), screen.get_height(), 0.6, 0.5, 10, 25
        )
        for x in range(self.middle_scroller.sprite_amount):
            self.middle_scroller.add_new(
                random.randint(0, screen.get_width()),
                random.randint(0, screen.get_height()),
            )

        self.back_scroller = StarScroller(
            screen.get_width(), screen.get_height(), 0.01, 0.1, 20, 57
        )
        for x in range(self.back_scroller.sprite_amount):
            self.back_scroller.add_new(
                random.randint(0, screen.get_width()),
                random.randint(0, screen.get_height()),
            )
